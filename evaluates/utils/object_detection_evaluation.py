...
class ObjectDetectionEvaluation(object):
    ...
    def evaluate(self, cate):
        ...
        for class_index in range(self.num_class):
            ...
            # draw the AP plot for each category
            import matplotlib.pyplot as plt
            if (category_index[class_index-offset]['id'] != class_index+1):
                offset = offset+1
            else:
                cate_label = category_index[class_index-offset]['name']
        ...
        if self.num_class==21:
            # save the AP plot
            plt.savefig('result/AP_Plot_JHMDB.png',bbox_inches='tight')
        else:
            plt.savefig('result/AP_Plot_AVA.png',bbox_inches='tight')
        ...
        # draw histogram of AP for JHMDB, AVA has too much categories to draw histogram
        if self.num_class == 21:
            bar_ap = [0 if np.isnan(x) else x for x in self.average_precision_per_class]
            plt.bar(range(len(self.average_precision_per_class)),bar_ap,tick_label = [label['name'] for label in category_index])
            plt.xticks(rotation=90)
            plt.xlabel('Action Category')
            plt.ylabel('AP (Average Precision)')  
            plt.savefig('result/AP_Histogram_JHMDB.png',bbox_inches='tight')
