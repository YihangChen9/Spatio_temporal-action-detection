...
from utils.misc import (NestedTensor, nested_tensor_from_tensor_list,
                       accuracy, accuracy_sigmoid, get_world_size,
...
class TubeR(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        ...
        # get feature information from backbone algorithm， where pos is position embedding,xt is origin image tesor
        features, pos, xt = self.backbone(samples)
        src, mask = features[-1].decompose()
        assert mask is not None

        # feed the output of backbone into transformer to get result
        # input_proj is the transfer the feature from size of backbone'output to transformer's input
        hs = self.transformer(self.input_proj(src), mask, self.query_embed.weight, pos[-1])[0]

        # get binary result by fc layer
        if self.dataset_mode == 'ava':
        ...
        # project origin image tensor to size of transformer's output
        src_c = self.class_proj(xt)

        # resize the output of transfomer for further cross-attention for classification
        hs_t_agg = hs.contiguous().view(lay_n, bs, 1, nb, dim)

        # from origin image tensor get temporal context
        ...
        # use cross attention to combine the temporal context
        ...
        # use fc layer to get the prediction
...
def build_model(cfg):
    ...
    # build post processor to process result
